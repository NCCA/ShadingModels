...
void NGLScene::initializeGL()
{
  ...
  material m;
  m.Ka.set(0.4f,0.4f,0.4f);
  m.Kd.set(1.0f,1.0f,1.0f);
  m.Ks.set(1.0f,1.0f,1.0f);
  m.shininess=20.0f;
  GLuint matUBOIndex = shader->getUniformBlockIndex("material");
  // generate a buffer ID for the transfom
  GLuint matBufferID;
  glGenBuffers( 1, &matBufferID );
  glBindBuffer( GL_UNIFORM_BUFFER, matBufferID );
  std::cout<<"index "<<matUBOIndex<<" Mat Size "<<sizeof(material)<<"\n";
  glBufferData( GL_UNIFORM_BUFFER, sizeof(material),&m,GL_DYNAMIC_DRAW );
  glBindBufferBase( GL_UNIFORM_BUFFER, matUBOIndex, matBufferID );
  glUnmapBuffer(GL_UNIFORM_BUFFER);

  std::cout<<"material "<<matUBOIndex<<"\n";
//  shader->printProperties();
//  shader->setShaderParam3f("mat.Ka",0.4f,0.4f,0.4f);
//  // red diffuse
//  shader->setShaderParam3f("mat.Kd",1.0f,1.0f,1.0f);
//  // white spec
//  shader->setShaderParam3f("mat.Ks",1.0f,1.0f,1.0f);
//  shader->setShaderParam1f("mat.shininess",20.0f);
  ...
  {
    LightInfo l;
    l.position.set(positions[index],positions[index+1],positions[index+2]);
    l.Ld.set(colours[index],colours[index+1],colours[index+2]);
    l.Ls.set(colours[index],colours[index+1],colours[index+2]);
    l.La.set(0.0f,0.0f,0.0f);
    lights.push_back(l);
    index+=3;
  }
  GLuint lightUBOIndex = shader->getUniformBlockIndex("light");

  GLuint lightBufferID;
  glGenBuffers( 1, &lightBufferID );
  glBindBuffer( GL_UNIFORM_BUFFER, lightBufferID );
  std::cout<<"Light buffer id "<<lightBufferID<<"\n";
  glBufferData( GL_UNIFORM_BUFFER, sizeof(LightInfo)*lights.size(),&lights[0],GL_DYNAMIC_DRAW );
  glBindBufferBase( GL_UNIFORM_BUFFER, lightUBOIndex, lightBufferID );
  glUnmapBuffer(GL_UNIFORM_BUFFER);
